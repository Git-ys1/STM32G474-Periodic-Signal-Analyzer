...
float Vpp_Robust(const float *data, uint32_t len)
{
    ...
    /* 2. 插入排序(2048点对Cortex-M4够快)
       也可用 arm_sort_f32 (CMSIS-DSP 1.10+) */
    for (i = 1; i < len; i++) {
        key = sorted[i];
        j = i;
    ...
    /* 3. 取 1% 和 99% 分位数, 过滤极端噪声 */
		float idx_low=0 ;           /* 1% 位置 */
    float idx_high=0; /* 99% 位置 */
		for (i = 2; i < 5; i++) {
			idx_low+=sorted[i];
...
float Vpp_R()
{
			...
			uint32_t valid_len = (uint32_t)floor(exact_len);
			uint32_t discard_len = 2048 - valid_len;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		if(AdcConvEnd == 1)
		{
			...
			//arm_max_f32(VO, 2048, &vmax, &maxid);   /* 找最大值 */
			//arm_min_f32(VO, 2048, &vmin, &minid);   /* 找最小值 */
			float vpp =Vpp_Robust(VO,2048);                 /* 峰峰值 */
