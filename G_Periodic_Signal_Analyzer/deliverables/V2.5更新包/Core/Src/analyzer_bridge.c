 ...
 * STM32启动文件仅预留1 KB主栈，因此准备/发布真实结果时不能把它作为
 * AnalyzerBridge_PrepareReal()或PublishPreparedReal()的局部变量放到栈上。
 */
...
/*
 * 完整4096点VO电压缓冲区按基频相位折叠到256个显示相位槽。
...
/**
 * @brief 原地选择第k小浮点数，避免qsort递归和额外4096点副本。
 */
 ...
 *
 * 使用正余弦递推避免在4096点循环中反复调用sinf/cosf。这里只比较候选
...
/**
 * @brief 在普通折叠结果上计算Huber权重并对原VO电压样本执行第二遍折叠。
 *
 * 第一遍波形只用于预测每个样本的相位期望值；第二遍仍折叠原始中心化
 * VO样本。阈值为max(1.345×1.4826×MAD, 1.21 mV)，
 * 不增加全局平滑或高阶插值。
 */
...
/**
 * @brief 把Huber折叠波形投影到FFT已经识别出的整数谐波子空间。
 *
 * G题输入只包含基波和至多两种谐波。Huber能够抑制离群ADC样本，但不会
 * 删除256个相位槽之间仍然存在的非模型抖动。这里从队友FFT结果推导整数
 * 谐波次数，正交保留这些正弦/余弦系数，并删除其余频率成分。
 *
 * 这一步只重建显示波形，不修改Vpp、RMS、基频或队友FFT分量。工作区复用
 * s_phase_sum_mv，不增加千字节级静态RAM。
 */
static bool AnalyzerBridge_ApplySelectedHarmonicProjection(
    AnalyzerResult *result)
{
    uint8_t harmonic_orders[ANALYZER_MAX_COMPONENTS];
    uint8_t harmonic_count = 1U;
    float input_mean_mv = 0.0f;
    uint16_t i;
    uint8_t component;
    uint8_t harmonic;
    ...
    /*
     * 基波无条件保留；其余谱线只有在“频率/基频”足够接近整数时
     * 才进入显示模型。A组数据的8次谐波会命中相邻两个FFT峰，必须去重。
     */
         ...
         ++component)
    {
        ...
        {
            harmonic_orders[harmonic_count] =
                (uint8_t)candidate_order;
            ++harmonic_count;
        }
    }
    ...
    for (i = 0U; i < ANALYZER_DISPLAY_POINT_COUNT; ++i)
    {
        input_mean_mv += result->waveform_mv[i];
    }
    input_mean_mv /= (float)ANALYZER_DISPLAY_POINT_COUNT;
    memset(s_phase_sum_mv, 0, sizeof(s_phase_sum_mv));
         ...
         ++harmonic)
    {
        ...
        {
            float centered_mv =
                result->waveform_mv[i] - input_mean_mv;
            float next_real;

            cosine_sum += centered_mv * oscillator_real;
            sine_sum += centered_mv * oscillator_imag;
            next_real =
                oscillator_real * step_real -
                oscillator_imag * step_imag;
            oscillator_imag =
        ...
        sine_coefficient =
            2.0f * sine_sum /
            (float)ANALYZER_DISPLAY_POINT_COUNT;
        oscillator_real = 1.0f;
        oscillator_imag = 0.0f;
    ...
    for (i = 0U; i < ANALYZER_DISPLAY_POINT_COUNT; ++i)
    {
        input_mean_mv += s_phase_sum_mv[i];
    }
    input_mean_mv /= (float)ANALYZER_DISPLAY_POINT_COUNT;
    ...
    {
        result->waveform_mv[i] =
            s_phase_sum_mv[i] - input_mean_mv;
    }
...
{
    AnalyzerWaveformFoldMode previous_mode = s_waveform_fold_mode;
    ...
    /*
     * 测试源可立即重建同一帧；真实源不再缓存VO，下一帧PrepareReal()
     * 会自动使用新模式。这样既保留队友VO的唯一所有权，也避免额外
     * 缓存4096点浮点数组。
     */
    if (s_test_override)
    {
