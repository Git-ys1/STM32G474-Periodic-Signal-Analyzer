...
| `t_vpp2` | 2048点Huber谐波模型峰峰值 | 文本控件，临时对照用，初值`Mpp: --.- mV` |
...
| `t_vpp2` / `Mpp` | 全部2048点的Huber谐波拟合，在4096相位点重建后求峰峰值 | 真实ADC链路按前端总增益6折算回信号源输入端 |

`Mpp`不使用256点显示波形，也不受普通/Huber折叠开关影响；FFT谱峰只负责
选择允许进入模型的整数谐波次数，最终幅值和相位仍由2048个原始ADC样本重新
