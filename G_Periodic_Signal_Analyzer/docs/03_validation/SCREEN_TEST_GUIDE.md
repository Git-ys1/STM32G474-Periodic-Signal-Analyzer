...
- 短按：复用原1T/3T命令；
- 长按：复用原测试命令，进入随机测试并每3秒自动换组；
- 实体键和HMI命令必须经过同一个`Display_ProcessButtonCommand()`入口。

## 普通/增强Huber状态开关验证

V2.2定版后HMI状态开关`sw_huber`默认`val=1`。先确认以下帧：

```text
A5 02 08 00 5A  普通折叠
A5 02 08 01 5A  Huber折叠 + FFT已识别谐波投影
```
...
3. 切回普通，确认只重画时域；
4. 测试组也会按其已报告频谱做投影，确认只保留对应整数次谐波；
5. 进入真实模式，在出现斜坡小峰谷的稳定输入上快速往返切换并录屏；
6. 重点确认增强Huber侧斜坡不再上下细碎折返，而真实高次谐波峰谷仍保留；
7. 同步保存当前最新`VO[4096]`及双路来源顺序，再与PC端普通/增强Huber结果对照；
8. 若FFT存在伪峰或漏峰，先记录频谱与原始ADC，不得只保留好看的案例。

状态开关只测试显示波形重建，不应改变`Upp`、`Urms`、基频和谱线。若切换后
这些文字变化，应先区分后台真实ADC帧自然更新和算法越界；Huber与谐波投影
代码本身不写这些字段。
...
1. 确认`s_time=512×256`、`s_spec=256×256`，频谱右边界恰好为800；
2. 时域五档纵轴应为实际`+Mpp/2`到`-Mpp/2`，频谱五档纵轴应为实际mV值；
3. 时域横轴`x_tx0`固定0.0，`x_tx4`为完整显示时长，单位us；
4. KEY1短按1T→3T时，`x_tx1`～`x_tx4`应同时变为原来的3倍，
   `sw_period`外观应切到3T；再次短按应全部恢复；
5. 频谱横轴固定0、100、200、300、400、500 kHz，500 kHz刻度不超出屏幕；
6. 清除后纵轴恢复`---`，时域横轴恢复0.0；再次刷新后恢复实际数值；
7. 串口不得出现`0x1A`对象名错误，若出现则逐项核对
   `n_ty*`、`n_sy*`、`x_tx*`和`sw_period`对象名。
