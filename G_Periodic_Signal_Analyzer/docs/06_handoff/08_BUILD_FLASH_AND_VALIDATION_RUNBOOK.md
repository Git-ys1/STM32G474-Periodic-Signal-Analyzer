...
$hex = 'F:\Project\stm32G474VETx\TI\G_Periodic_Signal_Analyzer\firmware\ADC\MDK-ARM\ADC\ADC.hex'
...
A5 02 08 01 5A  V2.2增强Huber：Huber折叠 + 已识别谐波投影
```
...
增强Huber A/B必须用同一信号设置和同一原始ADC帧；切换前后
Upp、Urms、频率和频谱文本必须不变。若干净波形看起来重合属于正常现象。

实物不可触摸，因此最终现场操作重点验证KEY1：短按切换周期，长按至少1秒并
松开后进入随机测试自动刷新。PB8复用BOOT0，上电或复位时不要一直按住，避免
启动模式受影响。V1.9“长按刷新真实ADC”已经被V2.2现场要求取代。
