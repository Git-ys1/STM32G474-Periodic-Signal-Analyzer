...
A5 02 07 trigger_mode 5A
A5 02 08 huber_enabled 5A
```
...
7. 点击“刷新”，确认退出测试锁存并恢复真实ADC自动显示。

不可触摸实物屏使用KEY1时：

- 短按：复用原1T/3T命令；
- 长按：复用原刷新命令，退出测试锁存并恢复真实ADC自动显示；
- 实体键和HMI命令必须经过同一个`Display_ProcessButtonCommand()`入口。

## 普通/Huber状态开关验证

HMI状态开关`sw_huber`默认`val=0`。先确认以下帧：
...
1. 进入测试自动刷新，记下当前`[T编号]`；
2. 在下一次3秒自动换组前切到Huber，确认T编号、频谱和六项文本不变；
3. 切回普通，确认只重画时域；
4. T101～T108本身是干净ADC数组，曲线接近或完全重合属于正常结果；
5. 进入真实模式，在出现局部毛刺的稳定输入上快速往返切换并录屏；
6. 同步保存当前最新`adc_b[2048]`，再与PC端普通/Huber结果对照；
7. T103、T108等低覆盖场景允许无改善或轻微退化，不得只保留好看的案例。

状态开关只测试显示波形重建，不应改变`Upp`、`Urms`、基频和谱线。若切换后
这些文字变化，应先区分后台真实ADC帧自然更新和算法越界；Huber代码本身不写
这些字段。
