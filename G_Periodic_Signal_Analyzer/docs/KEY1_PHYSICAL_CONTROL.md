...
适用工程：`projects/g474_full_integration_test`

> 本文主体记录V1.4.1首次引入KEY1时的历史验收。V1.9.0保留相同硬件、中断、
> 消抖和统一命令入口，但长按动作已由“测试”改为“刷新真实ADC”：
> `Display_RequestRefresh()`。当前行为以V1.9.0源码和
> [永久交接文档](HANDOFF/04_HARDWARE_WIRING_AND_HMI.md)为准。
...
| 长按 | 至少1000 ms后松开 | V1.9.0执行原“刷新”按钮命令，恢复真实ADC |
...
2. 短按一次，从1T切换到3T；
3. 再短按一次，从3T切回1T；
4. V1.9.0长按至少1秒并松开，执行与原“刷新”按钮相同的真实ADC恢复；
