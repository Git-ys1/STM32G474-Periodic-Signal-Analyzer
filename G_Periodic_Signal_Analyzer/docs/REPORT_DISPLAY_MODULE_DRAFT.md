...
> 当前实现基线：`v2.1.0 / projects/g474_full_integration_test`
...
针对孤立ADC异常点污染相位槽并在794列显示中形成局部毛刺的问题，在PC端增加
...
2048点静态残差工作区和最近一帧ADC副本，构建为`0 Error(s), 0 Warning(s)`；
...
6. Huber相位折叠已经提供默认关闭的上板状态开关，但真实ADC毛刺的幅度、密度
   和连续性尚未确认；是否成为正式模式仍需用同步保存的`adc_b[2048]`复测。
...
误差和2 s响应指标。PC人工污染试验表明，两遍Huber折叠值得作为中高覆盖、
孤立毛刺场景的上板候选；当前固件已用默认关闭的状态开关提供普通/Huber
可视化对照。低覆盖和连续异常仍需维持普通链路及可信度保护，不能仅凭离线
或单张屏幕结果把Huber改成正式默认。
