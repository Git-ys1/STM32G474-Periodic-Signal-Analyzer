...
- 显示模块不修改上游给出的 \(U_{\mathrm{pp}}\)、\(U_{\mathrm{rms}}\)、频率和频谱幅值；V2.4仅在桥接层增加独立的2048点模型峰峰值 \(M_{\mathrm{pp}}\) 用于标定对照，不替换队友结果。
...
7. 可选用全部2048点做Huber谐波拟合，输出独立模型峰峰值；
...
- `Upp`：复合波形峰峰值，单位 mV；
- `Mpp`：2048点Huber谐波模型峰峰值，单位 mV，仅用于与`Upp`和信号源实时对照；
- `Urms`：复合波形真有效值，单位 mV；
- `f1`：基频，单位 kHz；
...
7. 谐波投影依赖FFT分量识别，伪谐波或漏检会直接影响保留的显示模型，不能用
   平滑后的外观代替频谱正确性判断。
8. 2048点模型`Mpp`同样依赖FFT选对谐波次数和前端增益标定；在完成实屏三方
...
V2.4进一步增加不经过256槽的2048点Huber谐波模型`Mpp`，用于和队友`Upp`及
